...
const AverageScoreCard: React.FC<AverageScoreCardProps> = ({ averageScore = 0, recentScores = [] }) => {
  ...
  return (
    <div className="col-span-1 md:col-span-12 lg:col-span-4 bg-surface-light dark:bg-surface-dark rounded-xl sm:rounded-2xl p-4 sm:p-5 md:p-6 shadow-card-light dark:shadow-card-dark border border-border-light dark:border-border-dark flex flex-col h-full min-h-0">
      {/* Header */}
      <div className="flex justify-between items-center mb-2 sm:mb-3 shrink-0">
        <h2 className="text-xs sm:text-sm font-medium text-text-secondary-light dark:text-text-secondary-dark flex items-center gap-1.5 sm:gap-2">
          <span className="material-symbols-outlined text-primary text-base sm:text-lg">bar_chart</span>
          Average Score
        </h2>
        <span className="px-1.5 sm:px-2 py-0.5 text-[8px] sm:text-[10px] font-medium text-primary bg-primary/10 rounded-full">
          Last 5 tests
        </span>
      </div>

      {/* Score Display */}
      <div className="mb-2 sm:mb-3 md:mb-4 shrink-0">
        <div className="flex items-baseline gap-0.5 sm:gap-1">
          <span className="text-2xl sm:text-3xl md:text-4xl font-bold text-text-light dark:text-text-dark">{animatedScore}</span>
          <span className="text-sm sm:text-base md:text-lg text-text-secondary-light dark:text-text-secondary-dark">/300</span>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2 mt-0.5 sm:mt-1">
          <div className="h-1 sm:h-1.5 flex-1 bg-border-light dark:bg-border-dark rounded-full overflow-hidden">
            <div
              className="h-full bg-gradient-to-r from-primary to-accent rounded-full transition-all duration-1000 ease-out"
              style={{ width: `${(animatedScore / 300) * 100}%` }}
            />
          </div>
          <span className="text-[10px] sm:text-xs font-medium text-primary">{Math.round((animatedScore / 300) * 100)}%</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-end min-h-0">
        <div className="relative h-full flex">
          {/* Y-Axis Labels */}
          <div className="flex flex-col justify-between h-full pr-1 sm:pr-2 shrink-0">
            {[...gridLines].reverse().map((val) => (
              <span key={val} className="text-[7px] sm:text-[9px] text-text-secondary-light dark:text-text-secondary-dark leading-none">
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="flex-1 relative">
              ...
              <div className="absolute inset-0 flex items-end justify-around px-1 sm:px-2">
                {scores.map((test, index) => (
                  <div key={index} className="flex flex-col items-center h-full group cursor-pointer w-[24px] sm:w-[28px] md:w-[32px]">
                    {/* Bar with tooltip */}
                    <div className="w-full h-full relative flex items-end">
                      {/* Tooltip */}
                      <div className="absolute -top-8 sm:-top-10 left-1/2 -translate-x-1/2 z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none">
                        <div className="bg-surface-light dark:bg-surface-dark text-text-light dark:text-text-dark text-[10px] sm:text-xs font-semibold px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg shadow-lg border border-border-light dark:border-border-dark backdrop-blur-sm">
                          {test.score}
                        </div>
                      </div>
                      ...
                      {test.isPrimary ? (
                        ...
                        <div
                          ...
                          style={{ height: '8%', minHeight: '12px' }}
                        >
                          <span className="text-[6px] sm:text-[8px] text-text-secondary-light dark:text-text-secondary-dark">—</span>
                        </div>
                      )}
                    </div>
                  </div>
            ...
            {/* X-Axis Labels */}
            <div className="flex justify-around px-1 sm:px-2 pt-1 sm:pt-2 shrink-0">
              {scores.map((test, index) => (
                <span
                  key={index}
                  className={`text-[8px] sm:text-[10px] text-center font-medium transition-colors w-[24px] sm:w-[28px] md:w-[32px]
                    ${test.isCurrent
