...
const Waitlist: React.FC = () => {
    ...
    if (isAuthenticated) {
        return (
            <div className="min-h-screen flex items-center justify-center bg-background-light dark:bg-background-dark grid-bg-light dark:grid-bg-dark text-slate-900 dark:text-white">
                <div className="bg-white/80 dark:bg-[#141414]/80 p-8 rounded-2xl shadow-xl w-full max-w-md text-center border border-slate-200 dark:border-white/10">
    ...
    return (
        <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden font-display bg-background-light dark:bg-background-dark grid-bg-light dark:grid-bg-dark text-slate-900 dark:text-white">
            {/* Navigation Header */}
            <header className="absolute top-0 w-full px-6 py-6 md:px-20 flex justify-between items-center z-10">
            ...
            {/* Main Content Area */}
            <main className="relative z-10 w-full max-w-2xl px-6 py-20 text-center">
                <h1 className="text-5xl md:text-7xl font-black text-slate-900 dark:text-white leading-tight tracking-tighter mb-6">
                    Register for <span className="text-primary">AIPT</span>
                </h1>
                <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 font-normal leading-relaxed mb-12 max-w-xl mx-auto">
                ...
                {error && (
                    <div className="mb-4 p-3 bg-red-100 border border-red-400 text-red-700 rounded-xl text-sm max-w-md mx-auto">
                        {error}
                    </div>
                )}

                {/* Registration Card */}
                <div className="bg-white/60 dark:bg-[#141414]/60 backdrop-blur-md p-10 rounded-3xl border border-slate-200 dark:border-white/10 shadow-xl">
                    <form onSubmit={handleSubmit} className="space-y-8 text-left">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <label className="cursor-pointer group">
                                    ...
                                    />
                                    <div className="py-4 px-2 rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-[#1A1A1A] text-slate-600 dark:text-slate-300 peer-checked:border-primary peer-checked:bg-primary peer-checked:text-white transition-all text-sm font-semibold text-center group-hover:border-primary/30">
                                        13th Mar
                                    </div>
                                </label>
                                <label className="cursor-pointer group">
                        ...
                        <div className="space-y-4">
                            <div className="relative group">
                                ...
                                <input
                                    className="w-full pl-12 pr-4 py-4 rounded-2xl bg-white dark:bg-[#1A1A1A] border border-slate-200 dark:border-white/10 focus:border-primary focus:ring-0 focus:bg-white dark:focus:bg-[#141414] transition-all text-slate-900 dark:text-white placeholder:text-slate-400 font-medium"
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className={`w-full bg-primary hover:bg-blue-600 text-white font-bold py-5 rounded-2xl transition-all transform active:scale-[0.99] shadow-lg shadow-primary/20 flex items-center justify-center gap-3 text-lg ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                        >
